...
<!-- <div class="flex-center">-->
        <h1 class="text-cyan-900 heading">User Profile</h1>
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '<th class="py-3 bg-cyan-800">FirstName</th>';
    echo '<th class="py-3 bg-cyan-800">LastName</th>';
    ...
    echo '<th class="py-3 bg-cyan-800">HOUSENO</th>';
    echo '<th class="py-3 bg-cyan-800">REGION</th>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td class="py-3 px-6 ">' . $row['houseno'] . '</td>';
        echo '<td class="py-3 px-6 ">' . $row['region'] . '</td>';
        echo '<td class="py-3 px-6"><button class="bg-cyan-800 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded"><a href="user_edit.php?id='. $row['id'] .'">Edit</a></button></td>';
        
        echo '<td class="py-3 px-6">';
        echo '<form id="deleteForm'.$row['id'].'" method="POST" action="user_delete.php">';
